    ...
    <div className="fixed inset-0 bg-[#000000] flex items-center justify-center z-50 animate-fadeOut">
      <div className="relative">
        <div className="w-16 h-16 relative animate-scaleUp">
          <div className="absolute inset-0 bg-gradient-to-tr from-orange-500/20 to-orange-500/40 rounded-xl blur-xl animate-pulse" />
          <div className="relative w-full h-full rounded-xl bg-[#0A0A0A] border border-orange-900/20 flex items-center justify-center">
            <Image 
              src="/logo.svg"
              alt="Logo"
              width={48}
        ...
        <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap">
          <p className="text-sm text-orange-300/70 animate-pulse">
            Loading Damonic Web Browser
          </p>
        </div>
      </div>
    </div>
...
const MinimizedTabs = ({ tabs, onRestore }: { tabs: MinimizedTab[]; onRestore: (tab: MinimizedTab) => void; }) => {
  ...
  return (
    <div className="fixed bottom-14 left-0 right-0 bg-[#030303]/95 backdrop-blur-sm border-t border-orange-900/20 z-40">
          ...
          <div className="flex flex-nowrap gap-2 px-2">
            {sortedTabs.map((tab) => (
              <div
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#0A0A0A] border border-orange-900/20 
                         hover:border-orange-500/30 hover:bg-orange-500/10 transition-all duration-300 group
                         animate-slideIn whitespace-nowrap cursor-pointer"
                style={{ animationDelay: '100ms' }}
              >
                <div className="h-1.5 w-1.5 rounded-full bg-orange-500/70 group-hover:bg-orange-400 transition-colors" />
                <span className="text-sm text-orange-300/70 group-hover:text-orange-300 transition-colors">
                  {tab.title}
                </span>
              </div>
            ))}
        ...
        {allTabs.map((tab, index) => {
          ...
          return (
            <div
              key={uniqueKey}
              className={cn(
                "flex items-center gap-2 px-2 py-1 min-w-[100px] max-w-[160px] h-7 border-r border-l border-t rounded-t-md border-orange-900/20",
                "hover:bg-orange-500/5 transition-all duration-300 group relative select-none",
                isActive ? "bg-[#0A0A0A] border-orange-500/20" : "bg-[#030303]"
              )}
            >
              <div 
              ...
              >
                ...
                <span className="text-xs text-orange-300/70 truncate flex-1 text-left">
                  {tab.title}
                </span>
              </div>
              <div
                ...
                className="flex-shrink-0 p-0.5 hover:bg-orange-500/10 rounded cursor-pointer"
              >
                <X className="h-3 w-3 text-orange-300/50 hover:text-orange-300 transition-colors" />
              </div>
            </div>
          );
        })}
...
const ProjectCard = ({ 
  ...
  return expanded ? (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col h-full">
          <div className="sticky top-0 flex items-center justify-between h-8 bg-[#030303] border-b border-orange-900/20 z-50">
            <BrowserTabs
              activeTab={id}
              tabs={minimizedTabs}
            ...
            <div className="flex-shrink-0 flex items-center gap-1 px-2 bg-[#030303]">
              <Button
                ...
                className="text-orange-50/70 hover:text-orange-400 hover:bg-orange-500/10 h-6 w-6"
              >
    ...
    >
      <div className={cn(
        "absolute inset-0 rounded-xl bg-orange-500/5 opacity-0 blur-2xl transition-opacity duration-500",
        isHovered && "opacity-100"
      )} />
      
      <Card className="relative bg-[#030303] border border-orange-900/20 rounded-xl overflow-hidden hover:border-orange-500/30 transition-all duration-500 group-hover:translate-y-[-2px]">
        <div className="absolute inset-0 bg-gradient-to-b from-orange-900/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        
        <header className="relative p-4 flex justify-between items-start">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <div className="h-1.5 w-1.5 rounded-full bg-orange-500/70" />
              <h3 className="text-base font-medium text-orange-50/90 group-hover:text-orange-50 transition-colors duration-300">
                {title}
              </h3>
            </div>
            <p className="text-sm text-orange-50/50 group-hover:text-orange-50/70 transition-colors duration-300 max-w-[90%] leading-relaxed">
              {description}
            </p>
          </div>
          <Button
            ...
            size="icon"
            onClick={onExpand}
            className="text-orange-50/40 hover:text-orange-400 hover:bg-orange-500/10 h-8 w-8"
          >
        ...
        <div className="relative px-4 pb-4">
          <div 
            className="relative rounded-lg overflow-hidden bg-[#030303] border border-orange-900/20 group-hover:border-orange-500/20 cursor-pointer"
          ...
          >
            ...
            >
              {isLoaded ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-orange-900/20 to-[#030303]">
                  <MousePointerClick className="h-8 w-8 text-orange-400/100 mb-3 animate-pulse" />
                  <span className="text-sm text-orange-400/100 font-medium">Hover to preview</span>
                </div>
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-[#030303] via-[#030303]/80 to-transparent z-10" />
            </div>
            <div className="absolute bottom-3 left-3 z-20">
              <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-[#030303] border border-orange-500/20 text-orange-300">
...
const Footer = () => (
  <div className="fixed bottom-0 left-0 right-0 bg-[#030303]/95 backdrop-blur-sm border-t border-orange-900/20 z-50">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="h-14 flex items-center justify-center">
        <a 
          ...
          className="text-orange-300/70 hover:text-orange-300 transition-colors duration-300 flex items-center gap-2 text-sm font-medium"
        >
          Team Damonic
          <ExternalLink className="h-3.5 w-3.5" />
        </a>
    ...
    <>
      ...
      <div className={cn(
        ...
        <div className="fixed inset-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-tr from-orange-900/[0.02] via-transparent to-orange-900/[0.02]" />
        </div>
        ...
        <div className="relative min-h-screen">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-4 sm:h-14 gap-4 sm:gap-0">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-[#0A0A0A] border border-orange-900/20 flex items-center justify-center group">
                <Image 
                  src="/logo.svg"
                  alt="Logo"
                  width={32}
                ...
                <div>
                  <h1 className="text-xl font-semibold text-white">Damonic Web Browser</h1>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="relative flex-1 sm:flex-initial">
                  <input
                    ...
                    className="h-8 w-full sm:w-48 md:w-64 bg-[#0A0A0A] border border-orange-900/20 rounded-lg px-3 text-sm text-white/70 
                             placeholder:text-white/30 focus:outline-none focus:border-orange-500/50 focus:ring-0
